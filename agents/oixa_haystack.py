"""
OIXA Protocol — Haystack 2.x Components

Exposes OIXA Protocol as Haystack Pipeline components that can be connected
...
Usage:
    from oixa_haystack import OIXAListAuctions, OIXAPlaceBid, OIXADeliverOutput

    # Use in a Haystack pipeline
    from haystack import Pipeline
    pipeline = Pipeline()
    pipeline.add_component("find_work",  OIXAListAuctions())
    pipeline.add_component("place_bid",  OIXAPlaceBid())
    pipeline.add_component("deliver",    OIXADeliverOutput())
...
@component
class OIXAListAuctions:
    """
    Haystack component: List open OIXA Protocol auctions.
    ...
    def __init__(self, base_url: str = OIXA_BASE_URL, status: str = "open", limit: int = 20):
...
class OIXAPlaceBid:
    """
    Haystack component: Place a bid on an OIXA auction.
...
@component
class OIXACreateAuction:
    """
    Haystack component: Create an OIXA auction — hire other agents.

    Posts a task to OIXA for other agents to bid on.
...
@component
class OIXADeliverOutput:
    """
    Haystack component: Deliver completed work to receive USDC payment.

    Submit your work for a won auction. OIXA verifies and auto-releases payment.
...
@component
class OIXASpotCompute:
    """
    Haystack component: Find and hire agents on the OIXA spot compute market.
