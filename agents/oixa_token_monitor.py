"""
OIXA Protocol — Token Monitor & Auto-Delegation

Monitors LLM API credit levels (OpenAI, Anthropic) and automatically
delegates tasks to OIXA Protocol when tokens run low.

When your token budget is depleted or running low, instead of failing,
your agent can seamlessly delegate work to OIXA spot compute market.

Installation:
    pip install httpx anthropic openai  # (whichever you use)

Usage:
    from oixa_token_monitor import TokenMonitor, oixa_auto_delegate
    ...
    # Manual delegation
    result = await oixa_auto_delegate(
...
logger = logging.getLogger("oixa.token_monitor")

OIXA_BASE_URL = "http://localhost:8000"


# ── Async OIXA API helpers ─────────────────────────────────────────────────────

async def _oixa_get(path: str, base_url: str = OIXA_BASE_URL) -> dict:
...
async def _oixa_post(path: str, data: dict, base_url: str = OIXA_BASE_URL) -> dict:
...
def _oixa_post_sync(path: str, data: dict, base_url: str = OIXA_BASE_URL) -> dict:
...
async def oixa_auto_delegate(
    ...
    """
    Automatically find and hire an OIXA agent for a task.
    ...
    """
    logger.info(f"[OIXA] Auto-delegating '{capability}' task for {requester_id}")

    # Find cheapest available spot agent
    spot_resp = await _oixa_get(
    ...
    if not listings:
        ...
        logger.info(f"[OIXA] No spot agents for '{capability}', creating auction...")
        auction_resp = await _oixa_post("/api/v1/auctions", {
    ...
    request_resp = await _oixa_post("/api/v1/spot/request", {
        "requester_id":    requester_id,
    ...
    logger.info(f"[OIXA] Delegated to {data.get('assigned_agent', '?')} at {data.get('agreed_price_usdc', '?')} USDC")
...
class TokenMonitor:
    """
    Monitor LLM token usage and auto-delegate to OIXA when credits run low.
    ...
    automatically routed to OIXA spot compute market.
            ...
            try:
                ...
                if "429" in err_str or "rate limit" in err_str or "quota" in err_str:
                    if _monitor.delegate_on_429:
                        logger.warning("[OIXA TokenMonitor] OpenAI rate limited — consider delegating via oixa_auto_delegate()")
                        _monitor._delegation_count += 1
                raise

        try:
            client.chat.completions.create = patched_create
            logger.info(f"[OIXA TokenMonitor] Wrapped OpenAI client for agent {self.agent_id}")
        except Exception as e:
            logger.warning(f"[OIXA TokenMonitor] Could not wrap OpenAI client: {e}")
        ...
        def patched_create(*args, **kwargs):
            try:
                ...
                if "overloaded" in err_str or "rate" in err_str or "529" in err_str:
                    if _monitor.delegate_on_429:
                        logger.warning("[OIXA TokenMonitor] Anthropic overloaded — consider delegating via oixa_auto_delegate()")
                        _monitor._delegation_count += 1
                raise

        try:
            client.messages.create = patched_create
            logger.info(f"[OIXA TokenMonitor] Wrapped Anthropic client for agent {self.agent_id}")
        except Exception as e:
            logger.warning(f"[OIXA TokenMonitor] Could not wrap Anthropic client: {e}")
    ...
    def _record_usage(
        ...
        if self.delegate_on_low and budget_used > (1 - self.low_threshold):
            logger.warning(
                f"[OIXA TokenMonitor] Budget {budget_used:.0%} used "
                f"(${self._total_cost_usd:.4f}/${self.daily_budget_usd}). "
                f"Consider delegating remaining tasks to OIXA spot market."
            )

    def get_stats(self) -> dict:
        """Return current usage stats and OIXA delegation recommendations."""
        ...
        return {
            ...
            "tip": (
                "Budget running low — use oixa_auto_delegate() to offload tasks to OIXA spot market."
    ...
    async def delegate_if_low(
        ...
        """
        Delegate a task to OIXA IF the token budget is running low.
        ...
        logger.info(f"[OIXA TokenMonitor] Budget low ({stats['budget_used_pct']}%) — auto-delegating")
        return await oixa_auto_delegate(
            task=task,
            capability=capability,
...
class NeMoTokenMonitor(TokenMonitor):
    ...
    Usage:
        ...
        if monitor.should_delegate_nim():
            return monitor.delegate_to_oixa_sync(task, "summarization")
    """
    ...
    def wrap_nim_client(self, client) -> None:
        """
        Wrap a NIM client (OpenAI-compatible) for token tracking and OIXA fallback.
    ...
    def should_delegate_nim(self) -> bool:
        """Return True if NIM token budget is running low and OIXA delegation is warranted."""
        return self.get_stats()["recommend_delegation"]

    def delegate_to_oixa_sync(self, task: str, capability: str) -> dict:
        """
        Synchronously delegate a task to OIXA when NIM token budget is low.
        Also notifies the OpenClaw swarm about the delegation.
        """
        result = _delegate_sync(task, capability, self.agent_id,
                                self.max_delegate_price, self.oixa_base_url)
...
def _delegate_sync(task: str, capability: str, agent_id: str,
    ...
    """Synchronous OIXA delegation helper."""
